...
let single_arg_get_many = [%rapper
  get_many(
    {sql|
      SELECT @int{id}, @string{username}
      FROM users
...
let get_single_function_out = [%rapper
  get_many(
    {sql|
...
let get_multiple_function_out = [%rapper
  get_many(
    {sql|
